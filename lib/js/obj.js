...
function extension_slot(x) {
  var slot = x.length !== undefined && Caml_obj.caml_obj_tag(x) !== 248 && x.length >= 1 ? x[0] : x;
  var name;
  if (slot.length !== undefined && Caml_obj.caml_obj_tag(slot) === 248) {
    name = slot[0];
  } else {
    throw Caml_builtin_exceptions.not_found;
  }
  if (Caml_obj.caml_obj_tag(name) === 252) {
    return slot;
  } else {
    throw Caml_builtin_exceptions.not_found;
  }
