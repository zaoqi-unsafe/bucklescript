...
function caml_update_dummy(x, y) {
  ...
  var y_tag = Caml_obj.caml_obj_tag(y);
  if (y_tag !== 0) {
    x.tag = y_tag;
    return /* () */0;
  } else {
  ...
  while(true) {
    ...
    if (a_type === "string") {
      ...
      if (is_a_number !== 0) {
        ...
        var tag_a = Caml_obj.caml_obj_tag(a);
        var tag_b = Caml_obj.caml_obj_tag(b);
        if (tag_a === 250) {
          _a = a[0];
          continue ;
...
function caml_obj_tag(a) {
  if (typeof a === "string") {
    return 252;
  } else if (typeof a === "number") {
    return 1000;
  } else {
    return Caml_obj.caml_obj_tag(a);
  }
}
